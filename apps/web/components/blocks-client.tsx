...
import { Search, Plus, LayoutGrid, SlidersHorizontal } from "lucide-react";
...
import type { Block, BlockType, BlockStatus } from "@/lib/types";
  ...
  function handleArchive(id: string) {
    setBlocks((prev) =>
      prev.map((b) => {
        ...
        toast.success(
          next === "archived" ? `"${b.title}" archived.` : `"${b.title}" restored.`
        );
    ...
    >
      ...
      <div className="relative ml-auto flex h-full w-full max-w-md flex-col bg-background shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {block.type === "Snippet" ? (
            <pre className="rounded-lg bg-muted p-4 text-xs text-foreground overflow-x-auto whitespace-pre-wrap font-mono">
              {block.content}
            </pre>
          ) : (
